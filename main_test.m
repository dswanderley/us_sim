...
alpha = [0, 1, 5, 10, 15, 20, 25, 30];
all_sim{length(alpha)} = [];
for a = 2:length(alpha)
    % Angle in rad - 0 for collinear
    max_angle = alpha(a) * pi /180;
    if (max_angle == 0)
    ...
    all_sim{a} = all_points;
    clear p_aux spatial_points array_dists s_points
%    close all
       
end

save sim_all.mat all_sim
